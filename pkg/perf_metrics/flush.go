...
func flushCompletedBuckets() {
	...
	hotBuckets.Range(func(key, value any) bool {
		...
		err := model.UpsertPerfMetric(&model.PerfMetric{
			...
			StreamOutputTokens: drained.streamOutputTokens,
			StreamGenerationMs: drained.streamGenerationMs,
		})
		if err != nil {
...
func redisCounters(values map[string]string) counters {
	return counters{
		...
		streamOutputTokens: parseRedisInt(values["s_out"]),
		streamGenerationMs: parseRedisInt(values["s_gen_ms"]),
	}
}
