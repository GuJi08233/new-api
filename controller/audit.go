...
func recordManageAuditFor(c *gin.Context, targetUserId int, action string, params map[string]interface{}) {
	...
	model.RecordOperationAuditLog(model.ClientLogSource(c), operatorUserId, auditContentEN(action, params), action, params, auditOperatorInfo(c), nil)
	markAuditLogged(c)
}
...
func recordUserSecurityAudit(c *gin.Context, userId int, action string, params map[string]interface{}) {
	model.RecordOperationAuditLog(model.ClientLogSource(c), userId, auditContentEN(action, params), action, params, nil, nil)
}
