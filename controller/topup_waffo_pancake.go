...
func WaffoPancakeWebhook(c *gin.Context) {
	...
	if isSubscription {
		...
		if err := model.CompleteSubscriptionOrder(model.ClientLogSource(c), tradeNo, string(bodyBytes), model.PaymentProviderWaffoPancake, ""); err != nil {
	...
	if err := model.RechargeWaffoPancake(model.ClientLogSource(c), tradeNo); err != nil {
