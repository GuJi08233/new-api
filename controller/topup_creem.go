...
func handleCheckoutCompleted(c *gin.Context, event *CreemWebhookEvent) {
	...
	if err := model.CompleteSubscriptionOrder(model.ClientLogSource(c), referenceId, common.GetJsonString(event), model.PaymentProviderCreem, ""); err == nil {
	...
	err := model.RechargeCreem(model.ClientLogSource(c), referenceId, customerEmail, customerName)
	if err != nil {
