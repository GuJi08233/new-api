...
const EditChannelModal = (props) => {
  ...
  const isConvertingToMultiKey = canConvertToMultiKey && batch && multiToSingle;
  ...
  const loadChannel = async () => {
    ...
    if (success) {
      ...
      data.pass_through_all_enabled =
        (data.pass_through_body_enabled && data.pass_through_headers_enabled) ||
        false;
      setChannelSettings({
  ...
  const submit = async () => {
    ...
    if (localInputs.type === 58) {
      ...
      settings.oa2_base_url_openai = (localInputs.oa2_base_url_openai || '')
        .trim()
        .replace(/\/+$/, '');
      settings.oa2_base_url_claude = (localInputs.oa2_base_url_claude || '')
        .trim()
        .replace(/\/+$/, '');
      settings.oa2_base_url_codex = (localInputs.oa2_base_url_codex || '')
        .trim()
        .replace(/\/+$/, '');
      settings.oa2_base_url_gemini = (localInputs.oa2_base_url_gemini || '')
        .trim()
        .replace(/\/+$/, '');
    } else {
      // 非 OA2 渠道，清理相关设置
              ...
              <div className='space-y-4'>
                ...
                <div className='pt-3'>
                  ...
                  <Form.Switch
                    ...
                    extraText={t(
                      '将客户端请求头（如 User-Agent）透传给上游，Authorization 等敏感头始终被过滤',
                    )}
                  />
                  <Form.Switch
                    field='pass_through_all_enabled'
                    label={t('全部透传')}
                    checkedText={t('开')}
                    uncheckedText={t('关')}
                    onChange={(value) => handlePassThroughAllChange(value)}
                    extraText={t('一键同时开启请求体与请求头透传')}
                  />
                  <Form.Switch
                    field='pass_through_rewrite_model_enabled'
                    label={t('透传时应用模型重定向')}
                    checkedText={t('开')}
                    uncheckedText={t('关')}
                    onChange={(value) =>
                    ...
                    extraText={t(
                      '开启请求体透传时，按模型重定向仅改写请求体中的 model 字段',
                    )}
                  />
                  ...
                  <Form.InputNumber
                    field='retry_times'
                    label={t('渠道重试次数')}
                    placeholder={t('0 = 使用全局设置')}
                    min={0}
                    onChange={(value) =>
                      handleChannelSettingsChange('retry_times', value)
                    }
                    style={{ width: '100%' }}
                    extraText={t(
                      '该渠道失败后的重试次数，0 表示使用全局重试次数',
                    )}
                  />
                  <Form.Switch
                    field='retry_on_same_channel'
                    label={t('同渠道重试')}
                    checkedText={t('开')}
                    uncheckedText={t('关')}
                    onChange={(value) =>
                      handleChannelSettingsChange(
                        'retry_on_same_channel',
                        value,
                      )
                    }
                    extraText={t(
                      '开启后失败时在同一渠道上重试，不切换到其他渠道',
                    )}
                  />
                  <Form.Input
                    field='retry_status_codes'
                    label={t('重试错误码')}
                    placeholder={t('例如: 429,500-503')}
                    onChange={(value) =>
                      handleChannelSettingsChange('retry_status_codes', value)
                    }
                    showClear
                    extraText={t(
                      '指定哪些错误码触发重试，非空时覆盖全局设置，支持范围和逗号分隔',
                    )}
                  />
                  {inputs.retry_on_same_channel && (
                    <Form.Switch
                      field='retry_exhausted_transfer'
                      label={t('重试耗尽后转移')}
                      checkedText={t('开')}
                      uncheckedText={t('关')}
                      onChange={(value) =>
                        handleChannelSettingsChange(
                          'retry_exhausted_transfer',
                          value,
                        )
                      }
                      extraText={t(
                        '同渠道重试耗尽后是否转移到其他渠道，关闭则直接返回错误',
                      )}
                    />
                  )}
                  ...
                  <Form.Input
                    field='auto_disable_status_codes'
                    label={t('自动禁用错误码')}
                    placeholder={t('例如: 401,429')}
                    onChange={(value) =>
                      handleChannelSettingsChange(
                        'auto_disable_status_codes',
                        value,
                    ...
                    showClear
                    extraText={t(
                      '命中这些错误码时自动禁用（多密钥渠道只禁用当前密钥）。非空时替换全局自动禁用状态码，且不依赖全局「失败时自动禁用」开关，但仍需开启上方「是否自动禁用」。支持范围和逗号分隔',
                    )}
                  />
                  <Form.Switch
                    field='auto_recovery_enabled'
                    label={t('定时测试并自动恢复')}
                    checkedText={t('开')}
                    uncheckedText={t('关')}
                    onChange={(value) =>
                    ...
                    extraText={t(
                      '按下方间隔定时测试被自动禁用的密钥（多密钥渠道逐把测试）或被自动禁用的渠道本身，测试通过即自动恢复；不依赖全局定时测试与「成功时自动启用」开关',
                    )}
                  />
                  {inputs.auto_recovery_enabled && (
                    <Form.InputNumber
                      field='auto_recovery_interval_minutes'
                      label={t('自动恢复测试间隔（分钟）')}
                      placeholder={t('0 = 10 分钟')}
                      min={0}
                      max={1440}
                      onChange={(value) =>
                        handleChannelSettingsChange(
                      ...
                      style={{ width: '100%' }}
                      extraText={t(
                        '每把被禁用的密钥（或渠道本身）至少间隔这么久才重测一次，0 表示 10 分钟，最大 1440',
                      )}
                    />
                  )}
                  <Form.Switch
                    ...
                    extraText={t(
                      '在测试提示词里附加当前时间，使每次测试的请求体都不同，避免上游网关返回缓存响应把已失效的密钥测成可用；对手动测试、全局定时测试与自动恢复一并生效；全局监控设置中已开启时无需在此单独开启',
                    )}
                  />
                  ...
                  <Form.InputNumber
                    ...
                    min={0}
                    onChange={(value) =>
                      handleChannelSettingsChange('daily_request_limit', value)
                    }
                    style={{ width: '100%' }}
                    extraText={
                      t(
                        '该渠道每天最多承接的请求次数（所有模型合计，含重试与渠道测试）。达到上限后当天自动跳过该渠道，次日恢复',
                      ) +
                      (isEdit
                        ? '，' + t('今日已用') + ': ' + todayRequestCount
                        : '')
                    }
                  />
                  <Form.Select
                    ...
                    style={{ width: '100%' }}
                    extraText={t(
                      '按所选地区的当地 0 点重置并自动处理夏令时，也可直接输入其它 IANA 时区名。Google AI Studio 免费层级的 RPD 在太平洋时间午夜重置，请选「美国太平洋时间」。设置后下方固定 UTC 偏移不再生效',
                    )}
                  />
                  {!inputs.daily_request_limit_timezone && (
                    <Form.Select
                      ...
                      style={{ width: '100%' }}
                      extraText={t(
                        '每日计数在该时区的 0 点重置。上游免费额度按哪个时区重置就选哪个（如 UTC+8 = 北京时间），默认跟随服务器时区',
                      )}
                    />
                  )}
                  ...
                  <Form.InputNumber
                    field='rate_limit_period_minutes'
                    label={t('限制周期（分钟）')}
                    placeholder={t('0 = 1 分钟')}
                    min={0}
                    max={1440}
                    onChange={(value) =>
                      handleChannelSettingsChange(
                    ...
                    style={{ width: '100%' }}
                    extraText={t(
                      '按自然周期计数，如 1 分钟即每个整分钟重置。这是渠道级限制，与用户/分组级的模型请求限速相互独立',
                    )}
                  />
                  <Form.InputNumber
                    ...
                    style={{ width: '100%' }}
                    extraText={t(
                      '该渠道每周期最多承接的请求次数，包括失败、重试与渠道测试，0 代表不限制。达到后本周期内自动跳过该渠道',
                    )}
                  />
                  <Form.InputNumber
                    field='rate_limit_max_success'
                    label={t('每周期最多请求完成次数')}
                    placeholder={t('0 = 不限制')}
                    min={0}
                    onChange={(value) =>
                      handleChannelSettingsChange(
                        'rate_limit_max_success',
                        value,
                      )
                    }
                    style={{ width: '100%' }}
                    extraText={t(
                      '只包括在该渠道上成功完成的次数，0 代表不限制。达到后本周期内自动跳过该渠道',
                    )}
                  />
                </div>
              </div>
