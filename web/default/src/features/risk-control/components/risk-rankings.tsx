...
export function RiskRankings() {
  ...
  const showLogWarning = meta && (!meta.ip_log_enabled || !meta.ua_log_enabled)
              ...
              {(items as IpRankItem[]).map((item) => (
                <TableRow key={item.ip}>
                  ...
                  <TableCell>
                    <Badge
                      variant={
                        item.user_count > 5 ? 'destructive' : 'secondary'
                      }
                    >
                      {item.user_count}
                    </Badge>
                ...
                <TableRow key={item.user_id}>
                  ...
                  <TableCell className="space-x-2">
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        openUserDetail(item.user_id, item.username)
                      }
                    >
                      {t('View IPs')}
                    </Button>
              ...
              <TableBody>
                {detail.items.map((item) => (
                  <TableRow key={item.user_id}>
                    ...
                    <TableCell>{item.request_count}</TableCell>
                    <TableCell>
                      {formatTimestampToDate(item.first_seen)}
                    </TableCell>
                    <TableCell>
