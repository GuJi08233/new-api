...
// setupProbeGuardTest 迁移相关表并重置 Probe Guard 内存状态、IP 封禁缓存与配置快照。
func setupProbeGuardTest(t *testing.T) {
	t.Helper()
...
func probeGuardContext(remoteIP string) *gin.Context {
...
func TestCheckProbeGuardDryRunOnlyAlerts(t *testing.T) {
	...
	c := probeGuardContext("203.0.113.10")
	require.NoError(t, CheckProbeGuard(c, "model-a"))
	require.NoError(t, CheckProbeGuard(c, "model-b"))
...
func TestCheckProbeGuardSkipsPrivateIpAndDisabled(t *testing.T) {
	...
	// 私网 IP 不参与检测
	c := probeGuardContext("192.168.1.5")
	require.NoError(t, CheckProbeGuard(c, "model-a"))
	require.NoError(t, CheckProbeGuard(c, "model-b"))
	...
	c2 := probeGuardContext("203.0.113.11")
...
func TestEscalateIpBanLadder(t *testing.T) {
	...
	action, err := EscalateIpBan("198.51.100.7", "违规一", model.IpBanSourceAutoRule)
	require.NoError(t, err)
	...
	action, err = EscalateIpBan("198.51.100.7", "违规二", model.IpBanSourceProbeGuard)
	require.NoError(t, err)
	...
	// 第三次违规:永久
	action, err = EscalateIpBan("198.51.100.7", "违规三", model.IpBanSourceAutoRule)
	require.NoError(t, err)
	...
	// 永久封禁后再触发:幂等,不再写新事件
	action, err = EscalateIpBan("198.51.100.7", "违规四", model.IpBanSourceAutoRule)
	require.NoError(t, err)
...
func TestCheckRequestRiskBlocksActiveBanRegardlessOfSwitch(t *testing.T) {
	...
	// 风控总开关关闭:动态封禁仍生效
	c := probeGuardContext("203.0.113.77")
	...
	c2 := probeGuardContext("203.0.113.77")
