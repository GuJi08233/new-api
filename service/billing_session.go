...
func (s *BillingSession) Reserve(targetQuota int) error {
	...
	strictReserve := imageRequest || s.relayInfo.ForcePreConsume || s.relayInfo.RelayFormat == types.RelayFormatOpenAIRealtime
...
func (s *BillingSession) reserveFunding(delta int, strict bool) error {
	...
	case *SubscriptionFunding:
		if err := model.ReserveUserSubscriptionQuota(funding.requestId, int64(delta)); err != nil {
...
func (s *BillingSession) syncRelayInfo() {
	...
	if sub, ok := s.funding.(*SubscriptionFunding); ok {
		...
		info.SubscriptionQuotaSnapshot = sub.QuotaSnapshot
