...
//
// OutputTokens/GenerationMs cover every request, while StreamOutputTokens and
// StreamGenerationMs only accumulate streaming requests where the generation
// phase can be isolated from the time-to-first-token wait. Throughput derived
// from the streaming columns is comparable across models; the all-request
// columns are the fallback when a model is never called in streaming mode.
type PerfMetric struct {
	...
	StreamOutputTokens int64  `json:"-" gorm:"default:0"`
	StreamGenerationMs int64  `json:"-" gorm:"default:0"`
}
...
func UpsertPerfMetric(metric *PerfMetric) error {
	...
	return DB.Clauses(clause.OnConflict{
		...
		DoUpdates: clause.Assignments(map[string]interface{}{
			...
			"stream_output_tokens": gorm.Expr("perf_metrics.stream_output_tokens + ?", metric.StreamOutputTokens),
			"stream_generation_ms": gorm.Expr("perf_metrics.stream_generation_ms + ?", metric.StreamGenerationMs),
		}),
	}).Create(metric).Error
}
...
type PerfMetricSummaryBucket struct {
	...
	StreamGenerationMs int64  `json:"stream_generation_ms"`
}
...
func GetPerfMetricsSummaryBucketsAll(startTs int64, endTs int64, groups []string) ([]PerfMetricSummaryBucket, error) {
	...
	query := ReadDB().Model(&PerfMetric{}).
		Select("model_name, bucket_ts, SUM(request_count) as request_count, SUM(success_count) as success_count, SUM(total_latency_ms) as total_latency_ms, SUM(ttft_sum_ms) as ttft_sum_ms, SUM(ttft_count) as ttft_count, SUM(output_tokens) as output_tokens, SUM(generation_ms) as generation_ms, SUM(stream_output_tokens) as stream_output_tokens, SUM(stream_generation_ms) as stream_generation_ms").
		Where("bucket_ts >= ? AND bucket_ts <= ?", startTs, endTs)
	if groups != nil {
		if len(groups) == 0 {
			return summaries, nil
