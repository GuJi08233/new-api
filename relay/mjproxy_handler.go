...
func RelaySwapFace(c *gin.Context, info *relaycommon.RelayInfo) *dto.MidjourneyResponse {
	...
	billingApplied, billingErr := service.SettleMidjourneyTaskBilling(info, midjourneyTask, priceData.Quota, mjResp.StatusCode == 200 && mjResp.Response.Code == 1)
	if billingErr != nil {
...
func RelayMidjourneySubmit(c *gin.Context, relayInfo *relaycommon.RelayInfo) *dto.MidjourneyResponse {
	...
	if midjResponse.Code == 3 {
		...
		// 归档渠道由 model.UpdateChannelStatus 冻结，自动禁用会被拒绝
	...
	billingApplied, billingErr := service.SettleMidjourneyTaskBilling(relayInfo, midjourneyTask, priceData.Quota, consumeQuota && midjResponseWithStatus.StatusCode == 200)
	if billingErr != nil {
		common.SysLog("error consuming Midjourney quota: " + billingErr.Error())
	}
	if billingApplied {
